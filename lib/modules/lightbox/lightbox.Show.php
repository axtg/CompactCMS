...
<!-- lay-out -->
<?php if(!isset($_GET['id'])&&empty($_GET['id'])&&isset($albums)&&count($albums)>1) { ?>
	<?php if(!empty($albums)) {
...
} elseif(isset($_GET['id'])&&!empty($_GET['id'])||isset($albums)&&count($albums)==1) {
	$album = (isset($_GET['id'])?htmlentities($_GET['id']):$albums[0]);
	echo "<h3>".$ccms['lang']['album']['album']." ".ucfirst($album)."</h3>";
	if(isset($_GET['id'])) { echo "<a href=\"".$cfg['rootdir'].$_GET['page'].".html\"\">".$ccms['lang']['backend']['tooverview']."</a><br/>"; }
	...
	if(isset($_GET['id'])) { echo "<p style=\"clear:both;\"><a href=\"".$cfg['rootdir'].$_GET['page'].".html\">".$ccms['lang']['backend']['tooverview']."</a></p>"; }
}?>