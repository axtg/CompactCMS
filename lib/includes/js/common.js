...
window.addEvent('domready', function(){
			...
			el.addEvent('click',function() {
				...
				input.addEvent('blur', function() {
					...
					var request = new Request.HTML({
						...
						method:'post',
						update: el,
						data: 'action=liveedit&id='+el.get('id')+'&content='+escape(content),
						onRequest: function() {
	...
	*/	
	...
	// Collapsible - add page
	var myAdd = new Fx.Slide('form_wrapper');
	myAdd.toggle();
	 
	$('toggle-1').addEvent('click', function(e){
		e = new Event(e);
		myAdd.toggle();
		e.stop();
	});
	
	// Collapsible - menu depth
	var myMenu = new Fx.Slide('menu_wrapper');
	renderList();
	myMenu.toggle();
		 
	$('toggle-3').addEvent('click', function(e){
		e = new Event(e);
		myMenu.toggle();
		e.stop();
	});
