...
function SetUpLanguageAndLocale($language)
{
	...
	if(is_file($langfile))
	{
		...
		if ($language !== @$cfg['language']) // [XG] Added @ before $cfg['language'] due to thrown notice
		{
...
$ccms['printing'] = (isset($_GET['printing'])?filterParam4IdOrNumber($_GET['printing']):'N');
if ($ccms['printing'] != 'Y')
{
	$ccms['printing'] = 'N';
}
...
if($current != "sitemap.php" && $current != "sitemap.xml" && $pagereq != "sitemap") 
{
	...
	function ccmsContent($page,$published) 
	{
		...
		*/
		...
		$msg = @explode(' ::', $ccms['lang']['hints']['published']); //[XG] Added @ for undefined notice on $ccms['lang']
		ob_start();
